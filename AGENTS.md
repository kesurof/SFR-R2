...
3. Vérifier si une dépendance déjà installée couvre correctement le besoin.
4. Réutiliser l'existant lorsqu'il couvre le contrat demandé.
5. Étendre une abstraction lorsqu'au moins deux usages partagent réellement le
   même besoin.
6. Comparer avec une bibliothèque externe éprouvée avant de créer du code maison.
7. Créer une nouvelle abstraction uniquement lorsqu'aucune solution existante
   n'est adaptée.
8. Ne pas généraliser un composant métier sans duplication réelle.
9. Mettre à jour le catalogue lorsqu'un élément devient partagé, change de contrat
   ou cesse de l'être.
10. Ajouter ou mettre à jour les tests et vérifications correspondant au contrat
    de l'abstraction.

Ces règles impliquent également que :

- le nombre de lignes ou de fichiers ne justifie jamais à lui seul une abstraction ;
- un composant ne doit pas être rendu générique pour un usage hypothétique ;
- la composition est préférable à l'accumulation de propriétés conditionnelles ;
- un wrapper qui ne fait que renommer l'API d'une bibliothèque est à éviter ;
- chaque comportement doit conserver une source de vérité unique ;
- une abstraction devenue inutile doit être supprimée après migration ;
- le code, les tests et la documentation doivent rester synchronisés.
