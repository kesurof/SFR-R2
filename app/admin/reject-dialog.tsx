...
export function RejectDialog({ requestId, action = decide, commentField = "rejectionReason" }: { requestId: string; action?: RejectAction; commentField?: string }) {
  ...
  return <>
    ...
    <dialog ref={ref} className="details-dialog" onClick={(e) => { if (e.target === e.currentTarget) ref.current?.close(); }}>
      <div className="details-sheet"><div className="details-head"><div><span className="eyebrow">Décision</span><h3>Refuser la demande</h3></div><button type="button" className="icon-btn" aria-label="Fermer" onClick={() => ref.current?.close()}>×</button></div>
        <form action={action} className="panel-body stack">
          <input type="hidden" name="requestId" value={requestId} /><input type="hidden" name="decision" value="reject" />
