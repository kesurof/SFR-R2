...
| Composant/helper | Chemin | Responsabilité et API principale | Usages actuels | Contraintes | Tests associés |
| --- | --- | --- | --- | --- | --- |
| `DiscordUserColumns` | `app/components/discord-user-columns.tsx` | Rend les en-têtes, filtres et cellules d'identité Discord : pseudo, nom, ID, rôles et anciennetés. Props discriminées par `kind: "header" | "filters" | "cells"`. | `admin?view=users` et `demandes-acces`. | Réutiliser ce composant pour toute table affichant ces six colonnes ; ne pas recopier le markup ou les filtres. | `tests/discord-user-columns.test.ts`, `npm run typecheck`, `npm run build` |
