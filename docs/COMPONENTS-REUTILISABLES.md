...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `AppShell` | Composant partagé | `app/components/app-shell.tsx` | Structure globale Ant Design (`Layout`, `Sider`, `Menu`), navigation, compte et fil d'Ariane ; en-tête et `Sider` fixes ; `Drawer` latéral en présentation mobile. | `nav`, `account`, `signOutAction`, `children` | `app/layout.tsx` | Réserver à la structure globale ; `Header` et `Sider` desktop collants (`sticky`), navigation en `Drawer` sous le breakpoint `lg` ; ne pas y placer de logique métier de page. | Aucun test dédié ; `npm run typecheck`, `npm run build` |
...
| `useIsMobile` | Hook partagé | `app/components/use-is-mobile.ts` | Indique la présentation mobile sous le breakpoint `lg` (992 px), en desktop-first (valeur `false` au premier rendu, mise à jour après montage). | `useIsMobile(): boolean` | `AppShell`, tables converties en cartes et overlays responsives. | Ne pas lire `matchMedia` ailleurs ; conserver le seuil `lg` ; le desktop ne doit jamais être rendu en version mobile. | `tests/use-is-mobile.test.tsx` |
| `MobileCardList` | Composant partagé | `app/components/mobile-card-list.tsx` | Liste de cartes empilées pour la présentation mobile, avec pagination optionnelle. | `items`, `rowKey`, `pageSize?`, `emptyText`, `renderItem` | `KeysTable`, `SponsorshipTable`, `AccessRequestTable`, `RequestTable`, `UserTable`. | Réserver à la présentation mobile ; ne pas y mettre de logique métier ; préféré à `List` (déprécié par Ant Design 6.6). | Aucun test dédié ; `npm run typecheck`, `npm run build` |
| `MobileCardField` | Composant partagé | `app/components/mobile-card-field.tsx` | Ligne libellé / valeur d'une carte mobile, pour homogénéiser les cartes. | `label`, `children`, `align?` | Cartes `KeysTable` et `UserTable`. | Présentation pure ; ne pas y placer d'action ni de logique métier. | Aucun test dédié ; `npm run typecheck`, `npm run build` |
...
| `RejectDialog` | Composant partagé d'administration | `app/admin/reject-dialog.tsx` | `Modal` Ant Design de saisie d'un motif de refus et soumission d'une action ; utilisable en mode contrôlé sans déclencheur. | `requestId`, `requestIdField`, `action`, `commentField`, `title`, `open?`, `onOpenChange?`, `hideTrigger?`, `triggerLabel?`, `triggerIcon?` | Demandes de parrainage, demandes d'accès et remplacements de clés (dont `KeyActions`). | Réutiliser pour une décision nécessitant un motif ; conserver les champs d'identifiant et de commentaire adaptés à l'action. | Aucun test dédié ; `npm run typecheck`, `npm run build` |
| `ReplaceKeyDialog` | Composant partagé d'administration | `app/admin/replace-key-dialog.tsx` | `Modal` contrôlée de saisie d'une nouvelle clé, pour un remplacement direct (`keyId`) ou une demande membre (`replacementRequestId`). | `open`, `onClose`, `idValue`, `action?`, `idField?`, `title?`, `confirmLabel?`, `placeholder?` | `KeyActions`. | Ne jamais afficher la clé en clair reconnectée ; conserver `ANTI_AUTOFILL_PROPS` sur le champ secret. | Aucun test dédié ; `npm run typecheck`, `npm run build` |
| `RevokeKeyDialog` | Composant partagé d'administration | `app/admin/revoke-key-dialog.tsx` | `Modal` contrôlée de confirmation de révocation d'une clé active. | `open`, `onClose`, `keyId` | `KeyActions`. | Réserver à la révocation d'une clé active ; ne pas dupliquer le message métier. | Aucun test dédié ; `npm run typecheck`, `npm run build` |
| `DeleteKeyDialog` | Composant partagé d'administration | `app/admin/delete-key-dialog.tsx` | `Modal` contrôlée de suppression définitive, avec case « irréversible » obligatoire avant validation. | `open`, `onClose`, `keyId`, `fingerprint`, `status` | `KeyActions`. | Ne pas activer la confirmation avant la case à cocher ; rappeler les conséquences. | `tests/delete-key-dialog-render.test.tsx` |
...
| Règles de remplacement de clé | Helper métier partagé | `lib/key-replacement-rules.ts` | Statuts, validation des motifs, rattachement des demandes aux clés et filtres de la vue des clés. | `KEY_REPLACEMENT_REASON_MAX`, `keyReplacementInputError`, `keyReplacementDecisionError`, `buildKeyReplacementViews`, `matchesKeyFilters`, `KeyTableFilters` | Workflow de remplacement, actions serveur et `KeysTable`. | Ne jamais déplacer la validation uniquement côté client. | `tests/key-replacement.test.ts` |
| Origine d'une clé | Helper métier partagé | `lib/key-origin.ts` | Détermine l'origine d'une clé (demande directe, parrainage, restauration manuelle) depuis les journaux d'audit, en remontant les remplacements. | `resolveKeyOrigin`, `previousKeyIdFromMetadata`, `KeyOrigin`, `KeyOriginAudit` | `app/admin/page.tsx` (colonne `Origine`). | Fonction pure, sans accès base ni React ; audits comme source de l'origine. | `tests/key-origin.test.ts` |
| Cycle de vie des clés | Helper métier partagé | `lib/key-lifecycle.ts` | Effets transactionnels partagés par les parcours de révocation/rotation : statuts `KEY_READY` → `KEY_REVOKED` et rejet des demandes de remplacement orphelines. | `markKeyReadyRequestsRevoked`, `rejectPendingReplacements`, `notifyRejectedReplacements`, `REPLACEMENT_CANCELLED_COMMENT` | `lib/workflow.ts`, `lib/access-request-workflow.ts`, `lib/key-replacement-workflow.ts`. | Utiliser dans la transaction de révocation ; notifier après le commit uniquement. | Aucun test dédié ; `npm run typecheck`, `npm run build` |
| Étape d'accès du membre | Helper métier partagé | `lib/mon-access-rules.ts` | Combine parrainage et demande d'accès en une étape unique et fournit la position dans le parcours. | `computeAccessStage`, `ACCESS_STAGE_STEP`, `AccessStage` | `app/mon-acces/page.tsx`. | Conserver la précédence documentée (`ready` > `revoked` > `approved` > `pending` > `rejected`). | `tests/mon-access-rules.test.ts` |
| Normalisation des membres Discord | Helper métier partagé | `lib/discord-members.ts` | Extrait les champs utiles d'un membre Discord, sans dépendance réseau ni base. | `parseDiscordMember`, `DiscordMember`, `ParsedDiscordMember` | `lib/discord.ts`. | Garder la fonction pure ; le réseau et la persistance restent dans `lib/discord.ts`. | `tests/discord-sync.test.ts` |
...
| `KeysTable` | Composant métier | `app/admin/keys-table.tsx` | `Table` Ant Design de l'historique des clés (remplacement en lecture seule), avec recherche et filtre de statut persistés ; cartes sous `lg` ; la colonne Actions délègue à `KeyActions`. | `tests/keys-table-render.test.tsx`, `npm run typecheck`, `npm run build` |
| `KeyActions` | Composant métier | `app/admin/key-actions.tsx` | Action principale (`Remplacer`/`Traiter`) et menu `⋯` (`Révoquer`, `Refuser`, `Supprimer`) d'une ligne de clé, avec les modales associées. | `tests/key-actions-render.test.tsx`, `npm run typecheck`, `npm run build` |
| `SponsorshipTable` | Composant métier | `app/parrainer/suivi/sponsorship-table.tsx` | `Table` Ant Design du suivi des parrainages d'un membre ; cartes sous `lg`. | Aucun test dédié ; `npm run typecheck`, `npm run build` |
