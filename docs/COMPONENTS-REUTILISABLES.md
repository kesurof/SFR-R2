...
| `KeysTable` | Composant métier | `app/admin/keys-table.tsx` | `Table` Ant Design de l'historique des clés (remplacement en lecture seule) ; la colonne Actions délègue à `KeyActions`. | `tests/keys-table-render.test.tsx`, `npm run typecheck`, `npm run build` |
