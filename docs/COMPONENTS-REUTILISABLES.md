...
| Mutualisation potentielle | `ConfirmSubmit` et `RejectDialog` utilisent `Popconfirm` et `Modal` ; `RequestDetails` et `AccessRequestDetails` restent sur des dialogues natifs tant qu'ils ne sont pas migrés. | Ne pas fusionner leurs contrats : confirmation destructive, motif obligatoire et affichage de détail sont des responsabilités différentes. |
