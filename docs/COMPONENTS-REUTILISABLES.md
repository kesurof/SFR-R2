...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `AppShell` | Composant partagé | `app/components/app-shell.tsx` | Structure globale Ant Design (`Layout`, `Sider`, `Menu`), navigation, compte et fil d'Ariane ; `Drawer` latéral en présentation mobile. | `nav`, `account`, `signOutAction`, `children` | `app/layout.tsx` | Réserver à la structure globale ; `Sider` inchangé sur desktop, navigation en `Drawer` sous le breakpoint `lg` ; ne pas y placer de logique métier de page. | Aucun test dédié ; `npm run typecheck`, `npm run build` |
