...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `AppShell` | Composant partagé | `app/components/app-shell.tsx` | Structure globale Ant Design (`Layout`, `Sider`, `Menu`), navigation, compte et fil d'Ariane. | `nav`, `account`, `signOutAction`, `children` | `app/layout.tsx` | Réserver à la structure globale ; navigation dépliée par défaut sur grand écran et repliée sur mobile ; ne pas y placer de logique métier de page. | Aucun test dédié ; `npm run typecheck`, `npm run build` |
...
| `KeysTable` | Composant métier | `app/admin/keys-table.tsx` | `Table` Ant Design de l'historique des clés (remplacement en lecture seule), avec recherche et filtre de statut persistés ; la colonne Actions délègue à `KeyActions`. | `tests/keys-table-render.test.tsx`, `npm run typecheck`, `npm run build` |
