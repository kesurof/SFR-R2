...
describe("protection des secrets", () => {
  ...
  it("déchiffre aussi les secrets utilisés par le worker webhook", async () => { const secret = "https://discord.com/api/webhooks/100000000000000003/synthetic-token"; await expect(decryptWebhookSecret(encrypt(secret))).resolves.toBe(secret); });
